...
function readmore_hide(){ // Skills 더보기 텍스트교체 event
    var readmore = document.getElementsByClassName('skills_readmore')
    if(readmore_on==false){
        readmore[0].innerHTML = "스킬 접기 <span class='skills_readmore_plus'>↑</span>"
        readmore_on = true
    }else{
...
function skillsshow(e){ // for mobile
...
function nav_scroll(e){ // Nav_menu 스크롤 이동
...
function nav_mobile(){ // for mobile
...
function scroll_animation(){ // 스크롤 이벤트리스너
    ...
    if(scrolltop >= header[0].offsetTop){
        for(i=0;i<nav_menu.length;i++){ // menu색깔 초기화
    ...
    for(i=0;i<3;i++){
        if(scrolltop >= section[i].offsetTop){
            for(j=0;j<nav_menu.length;j++){  // menu색깔 초기화
                nav_menu[i].style.color = "#FFFFFF"}
